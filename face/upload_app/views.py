    ...
    if is_ajax(request):
        ...
        try:
            ...
            if False:  # active != True:
                logger.error(str(datetime.datetime.now())+ " " + "AJAX| inactive password in POST img64")
                return JsonResponse({'result': f'ERROR', 'msg': f'Заявка неактивна'})

        except Exception as e:
            
            logger.error(str(datetime.datetime.now())+ " " + "AJAX| exception in POST img64 ",e)
            ...
            try:
               ...
               # logger.error(str(datetime.datetime.now())+ " " + img64)
                ...
                try:
                    ...
                    logger.error(str(datetime.datetime.now())+ " " + responseServ)
                    ...
                    logger.error(str(datetime.datetime.now())+ " " + "error in server registering face")
                ...
                logger.error(str(datetime.datetime.now())+ " " + "Exception in converting img to base64")
        ...
        logger.error(str(datetime.datetime.now())+ " " + 'No Face found')
    ...
    if request.method == 'GET':
        ...
        if face_token_ch and len(face_token_ch) == 6:
            ...
            try:
                ...
                logger.error(str(datetime.datetime.now())+ " " + "exception in getting db data")
                return render(request, './upload_app/code.html',
                              {'value_pass': '007'})

        else:
            logger.error(str(datetime.datetime.now())+ " " + "exception in len password")
        ...
        if request.GET.get('master_password', False) == 'secretmasterpasswordvig':
            ...
            if face_token_ch and len(face_token_ch) == 6:
                ...
                try:
                    ...
                    logger.error(str(datetime.datetime.now())+ " " + "API|   Exception in database ")
                    return JsonResponse({'RESULT': 'ERROR', 'CODE': f'{e}', 'name':f'{0}'})
        else:
            logger.error(str(datetime.datetime.now())+ " " + "API|   Exception in secretpassword")
            return JsonResponse({'RESULT': 'AXAXAXAXAXAXAXXAX', 'CODE': 'Im not vindictive. I will write it down.', 'name':f'Your_MOTHER'})
    logger.error(str(datetime.datetime.now())+ " " + "API| exception in request type")
